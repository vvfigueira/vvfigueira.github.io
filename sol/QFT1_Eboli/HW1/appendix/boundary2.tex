...
The main Let our spacetime be a four dimensional manifold $\Omega$, with boundary $\partial\Omega = \Sigma_-\cup\Sigma_+\cup\Gamma$. 
...
\begin{align}
    ...
    \int\limits_{\tilde\Omega}\dd[4]{y}\fdv{S[\phi]}{\phi\qty(y)}\psi\qty(y)&=\int\limits_{\tilde\Omega}\dd[4]{y}\int\limits_\Omega\dd[4]{x}\qty[\pdv{\mathcal L[x]}{\phi\qty(x)}\delta^{(4)}\qty(x-y)+\pdv{\mathcal L[x]}{\partial_\mu\phi\qty(x)}\partial_\mu\delta^{(4)}\qty(x-y)]\psi(y)\\%&\quad\quad\quad+\int\limits_{\tilde\Omega}\dd[4]{y}\int\limits_{\partial\Omega}\dd[3]{S_{x\alpha}}\fdv{\ell^\alpha[x]}{\phi\qty(y)}\psi(y)\\
...
%For many cases $\ell^\alpha\equiv0$ due to dynamical and covariance reasons. 
Of course $\psi$ is zero on $\Sigma_\pm$, then, the equations of motion impose:
