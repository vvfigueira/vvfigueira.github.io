...
\begin{align}
    {\phi'}^I(x) &=\tensor{\exp\qty(-\im a^iT_i)}{^I_J}\phi^J\qty(x)\Rightarrow \dv{{\phi'}^I(x)}{a^i}\eval_{a^i=0}=-\im\tensor{T}{^I_J_i}\phi^J\qty(x)
\end{align}

This transformation generates a conserved current,
...
In the quantum theory, at least from the canonical quantization POV, we have the map $\comm{\hat A}{\hat B} = \im\hbar \acomm{A}{B}$, therefore, 
...
symmetries to quantum symmetries. Most of the anomalies are harmless, but, if there is an gauge anomaly the theory is physically inconsistent.
...
\begin{align}
    \acomm{\phi^I(x)}{Q_i}&= \int\dd[3]{\vb y}\qty(\fdv{\phi^I(t,\vb x)}{\phi^K\qty(\vb y)}\fdv{Q_i}{\pi_K\qty(\vb y)}-\fdv{\phi^I\qty(t,\vb x)}{\pi_K\qty(\vb y)}\fdv{Q_i}{\phi^K\qty(\vb y)})\\
    \acomm{\phi^I(x)}{Q_i}&= -\int\dd[3]{\vb y}\im\tensor{T}{^K_J_i}\phi^J\tensor{\delta}{^I_K}\delta^{(3)}\qty(\vb x-\vb y)\\
    \acomm{\phi^I(x)}{Q_i}&= -\im\tensor{T}{^I_J_i}\phi^J\equiv \dv{{\phi'}^I}{a^i}\eval_{a^I=0}\Rightarrow\comm{{\hat\phi}^I(x)}{\hat Q_i}= \hbar\tensor{T}{^I_J_i}{\hat\phi}^J
\end{align}
